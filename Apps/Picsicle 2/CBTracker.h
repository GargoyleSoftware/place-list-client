 ...
 *  Pass the account ID, and application id. For example:
 *
 *  \code
 *  [[CBTracker sharedTracker] startTrackerWithAccountID:1234 appid:@"myapp.example.com"];
 *  \endcode
 *
 ...
 * @param appid An application identifier. Should be structured like a
 *              domain name. Like \c ios.company.com
 *
 * See http://chartbeat.com/docs/account_id for your account id.
 */
- (BOOL)startTrackerWithAccountID:(int)uid appid:(NSString *)appid;

/**
 * Track a view.
 *
 * Passing in an actual view id is optional. If a view is passed in,
